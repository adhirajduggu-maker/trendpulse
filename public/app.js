...
function sourceBadge(t){
  ...
  else if(raw.includes("amazon")){name="Amazon";cls="source-amazon"}
...
function render(){
 ...
 <div class="score" style="--s:${t.score}"><span>${t.score}</span></div></div>`).join("")||`<div class="loading">${S.type==="product"?"No public product results were returned by Amazon, Flipkart or Meesho right now.":S.type==="video"?"No public video results were returned by YouTube, Instagram or Facebook right now.":"No matching opportunities."}</div>`;
