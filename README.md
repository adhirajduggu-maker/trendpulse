...
## V2.5 visual redesign
- Cards and buttons redesigned to match approved dashboard direction.
- Added metrics box, stronger watchlist, responsive action buttons, trust strip.

## V2.6 Real Images + Wide Layout
- Amazon parser now captures product images from actual search-result cards.
- JSON-LD product images are retained when available.
- YouTube results now include real video thumbnails.
- Instagram/Facebook public results attempt to retain thumbnails when present.
- UI displays real images first and falls back to a source icon only if an image is unavailable/broken.
- Desktop opportunity layout changed from cramped 4 columns to wide 2 columns.
- Tablet/mobile collapse to 1 column.
- Action buttons have fixed minimum widths and no clipping.
