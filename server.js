...
function genericLinks(html,host,label){
 const out=[],seen=new Set();
 for(const m of html.matchAll(/<a[^>]+href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){
  let text=m[2].replace(/<[^>]+>/g," ").replace(/\s+/g," ").trim();
  if(text.length<8||text.length>160||/sign in|account|cart|privacy|terms|help|menu/i.test(text))continue;
  let href=m[1];if(href.startsWith("/"))href=`https://${host}${href}`;if(!href.startsWith("http"))continue;
  const k=text.toLowerCase();if(seen.has(k))continue;seen.add(k);out.push({marketplace:label,title:text,url:href,rank:out.length+1});
  if(out.length>=25)break;
 } return out;
}
async function amazonSearch(domain,label,q){
 const html=await fetchText(`https://www.${domain}/s?k=${encodeURIComponent(q)}`);
 let rows=extractJsonLdProducts(html,label);if(rows.length)return rows;
 return genericLinks(html,domain,label);
}
...
async function argosSearch(q){
 ...
 let rows=extractJsonLdProducts(html,"Argos");if(rows.length)return rows;
 return genericLinks(html,"argos.co.uk","Argos");
}
...
app.get("/api/health",(q,r)=>r.json({ok:true,version:"2.4",countryAware:true,productSources:{IN:["Amazon India","Flipkart","Meesho"],US:["Amazon US","eBay US","Walmart"],GB:["Amazon UK","eBay UK","Argos"],CA:["Amazon Canada","eBay Canada","Walmart Canada"],AU:["Amazon AU","eBay AU"]},videoSources:["YouTube","Instagram","Facebook"]}));
...
app.listen(PORT,()=>console.log(`TrendPulse V2.4 running on ${PORT}`));
