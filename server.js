...
function extractAmazonSearchResults(html, domain, label){
 ...
 for(const card of cards){
   ...
   out.push({marketplace:label,title,url:href,rank:out.length+1});
   if(out.length>=30)break;
 }
 return out;
}
...
app.get("/api/health",(q,r)=>r.json({ok:true,version:"agent-v2.4-links-titles",countryAware:true,productSources:{IN:["Amazon India","Flipkart","Meesho"],US:["Amazon US","eBay US","Walmart"],GB:["Amazon UK","eBay UK","Argos"],CA:["Amazon Canada","eBay Canada","Walmart Canada"],AU:["Amazon AU","eBay AU"]},videoSources:["YouTube","Instagram","Facebook"]}));
