...
async function fetchText(url,headers={}){
 const r=await fetch(url,{headers:{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124 Safari/537.36",...headers},redirect:"follow"});
...
async function getProducts(geo="IN"){
 ...
 const queries=["trending","best seller","viral","new launch","deals"],all=[],status=[];
 for(const source of sources){
  let count=0;
  for(const q of queries){try{const r=await marketplaceSearch(source,q,geo);all.push(...r);count+=r.length}catch(e){console.warn("product",geo,source,e.message)}if(count>=12)break}
  status.push({source,available:count>0,count});
 }
 ...
 rows.sort((a,b)=>b.score-a.score);const data=rows.slice(0,40);data.sourceStatus=status;CACHE.set(key,{at:Date.now(),data});return data;
}
...
async function getVideos(geo="IN"){
 ...
 const queries=[`trending ${country}`,`viral ${country}`,`shorts ${country}`,`reels ${country}`];
 let raw=[];
 for(const q of queries){
   try{raw.push(...(await youtubeSearch(q,geo)));}catch(e){console.warn("youtube",e.message)}
 }
 // Direct public attempts only; no news/search-engine substitution.
 for(const platform of ["Instagram","Facebook"]){
   for(const q of [`trending ${country}`,`viral ${country}`]){
     const r=await publicPlatformSearch(platform,q);raw.push(...r);
     if(r.length>=8)break;
   }
 }
 const seen=new Set(),now=new Date().toUTCString(),rows=[];
...
app.get("/api/health",(q,r)=>r.json({ok:true,version:"2.6",countryAware:true,productSources:{IN:["Amazon India","Flipkart","Meesho"],US:["Amazon US","eBay US","Walmart"],GB:["Amazon UK","eBay UK","Argos"],CA:["Amazon Canada","eBay Canada","Walmart Canada"],AU:["Amazon AU","eBay AU"]},videoSources:["YouTube","Instagram","Facebook"]}));
...
function normGoal(g="Audience Growth"){return ["Audience Growth","Affiliate Sales","Traffic","Lead Generation"].includes(g)?g:"Audience Growth"}
...
function moneyAngle(t,goal){
 if(goal==="Affiliate Sales")return t.type==="product"?"Direct affiliate review/demo opportunity":"Use content to route viewers toward a related commercial offer.";
 if(goal==="Lead Generation")return "Use the trend as a lead magnet or problem-awareness entry point.";
 if(goal==="Traffic")return "Create search-led content with a clear CTA to a detailed page or tool.";
 return "Use the trend for reach first; monetize later through sponsors, affiliate links or your own offer.";
}
function actionPlan(t,goal){
 return {priority:t.score>=88?"Act now":t.score>=78?"Act today":"Test selectively",moneyAngle:moneyAngle(t,goal),nextSteps:[`Publish one short-form post around “${t.title}”.`,`Use this hook: ${(t.hooks||hooks(t.title))[0]}`,goal==="Affiliate Sales"?"Add a disclosed affiliate CTA where relevant.":"Use a simple CTA that matches your goal.","Track the signal again tomorrow and stop if momentum cools."]};
}
async function agentPool(geo){
 const search=await getSearchTrends(geo).catch(()=>[]);
 const products=await getProducts(geo).catch(()=>[]);
 const videos=await getVideos(geo).catch(()=>[]);
 return [...search.slice(0,20).map(x=>({...x,type:"search"})),...products.slice(0,20),...videos.slice(0,20)];
}
app.get("/api/agent/recommendations",async(q,r)=>{try{
 const geo=String(q.query.geo||"US").toUpperCase(), niche=q.query.niche||"All", platform=q.query.platform||"Any", goal=normGoal(q.query.goal);
 let pool=await agentPool(geo);
 ...
 r.json({geo,niche,platform,goal,fetchedAt:new Date().toISOString(),recommendations:ranked.slice(0,8)});
}catch(e){r.status(502).json({error:e.message})}});
...
app.listen(PORT,()=>console.log(`TrendPulse V2.6 running on ${PORT}`));
