...
function normalizeImage(v){
 ...
 if(typeof v==="object")return v.url||v.contentUrl||v.thumbnailUrl||"";
 return "";
}
   ...
   try{
     ...
     const walk=o=>{
       ...
       if((o["@type"]==="Product"||o["@type"]?.includes?.("Product"))&&o.name){
         ...
         if(name.length>3&&!seen.has(k)){seen.add(k);out.push({title:name,url,imageUrl:normalizeImage(o.image)});}
       }
...
function extractAmazonSearchResults(html, domain, label){
 const out=[],seen=new Set();

 // Amazon search result cards.
 const cards=[...html.matchAll(/<div[^>]+data-component-type=["']s-search-result["'][^>]*>([\s\S]*?)(?=<div[^>]+data-component-type=["']s-search-result["']|$)/gi)].map(m=>m[1]);

 for(const card of cards){
   let href="", title="";

   // Prefer h2 title links/spans only.
   let m=card.match(/<h2[^>]*>[\s\S]*?<a[^>]+href=["']([^"']+)["'][^>]*>[\s\S]*?<span[^>]*>([\s\S]*?)<\/span>[\s\S]*?<\/a>[\s\S]*?<\/h2>/i);
   if(!m){
     m=card.match(/<h2[^>]*>[\s\S]*?<span[^>]*>([\s\S]*?)<\/span>[\s\S]*?<\/h2>/i);
     if(m){
       title=cleanText(m[1]);
       const a=card.match(/<a[^>]+href=["']([^"']*\/(?:dp|gp\/product)\/[^"']+)["']/i);
       href=a?a[1]:"";
     }
   }else{
     href=m[1];
   ...
   if(!href||!/\/(?:dp|gp\/product)\//i.test(href))continue;
   ...
   const im=card.match(/<img[^>]+(?:data-src|src)=["']([^"']+)["'][^>]*>/i);
   const imageUrl=im?im[1]:"";
   out.push({marketplace:label,title,url:href,imageUrl,rank:out.length+1});
   if(out.length>=30)break;
 }
 return out;
}
...
async function amazonSearch(domain,label,q){
 ...
 let rows=extractAmazonSearchResults(html,domain,label);
 if(rows.length)return rows;

 // JSON-LD fallback, still restricted to product-detail URLs and validated names.
 rows=extractJsonLdProducts(html,label)
   .map(x=>({...x,title:normalizeProductTitle(x.title)}))
   .filter(x=>looksLikeProductTitle(x.title)&&/\/(?:dp|gp\/product)\//i.test(x.url||""));
...
function isBadProductTitle(t=""){
 const s=String(t||"").trim();
 if(!s)return true;
 if(/debug info copied|copied to clipboard|javascript|sign in securely|sponsored/i.test(s))return true;
 if(/^(?:[$£€₹]|Rs\.?|INR|USD|GBP|EUR|C\$|A\$)?\s*[\d,.]+(?:\s*(?:list|mrp|price|\/count|each|ea\.?))*[\s:;,$£€₹\d.()/-]*$/i.test(s))return true;
 if(/^(?:[$£€₹]\s*)?\d+(?:\.\d+)?\s*(?:[$£€₹]\s*)?\d+(?:\.\d+)?/i.test(s) && (s.match(/[A-Za-z]/g)||[]).length < 8)return true;
 if(/\/count\b/i.test(s) && (s.match(/[A-Za-z]/g)||[]).length < 12)return true;
 return false;
}
...
function looksLikeProductTitle(t=""){
 ...
 if(isBadProductTitle(s))return false;
 ...
 if(data)walk(data,o=>{
   ...
   if(v&&v.videoId&&v.title){
     ...
     if(title&&!seen.has(v.videoId)){
       ...
       const imageUrl=thumbs.length?thumbs[thumbs.length-1].url:"";
       out.push({platformName:"YouTube",title,sourceUrl:`https://www.youtube.com/watch?v=${v.videoId}`,imageUrl,viewText:v.viewCountText?.simpleText||"",publishedText:v.publishedTimeText?.simpleText||""});
     }
   }
 });
...
async function publicPlatformSearch(platform,query){
 ...
 try{
   ...
   for(const m of html.matchAll(re)){
     ...
     const im=m[2].match(/<img[^>]+(?:src|data-src)=["']([^"']+)["']/i);
     out.push({platformName:platform,title:text||`${platform} video result for ${query}`,sourceUrl:href.startsWith("http")?href:`https://www.${platform.toLowerCase()}.com${href}`,imageUrl:im?im[1]:""});
...
app.get("/api/health",(q,r)=>r.json({ok:true,version:"agent-v2.6-images-layout",countryAware:true,productSources:{IN:["Amazon India","Flipkart","Meesho"],US:["Amazon US","eBay US","Walmart"],GB:["Amazon UK","eBay UK","Argos"],CA:["Amazon Canada","eBay Canada","Walmart Canada"],AU:["Amazon AU","eBay AU"]},videoSources:["YouTube","Instagram","Facebook"]}));
