...
async function marketplaceSearch(source,query){
 const q=encodeURIComponent(query);
 if(source==="amazon"){
   const html=await fetchText(`https://www.amazon.in/s?k=${q}`);
   return extractAmazon(html);
 }
 if(source==="flipkart"){
   const html=await fetchText(`https://www.flipkart.com/search?q=${q}`);
   return extractFlipkart(html);
 }
 if(source==="meesho"){
   const html=await fetchText(`https://www.meesho.com/search?q=${q}`);
   return extractMeesho(html);
 }
 return [];
}
async function getProducts(){
 const key="products:IN",hit=CACHE.get(key);if(hit&&Date.now()-hit.at<TTL)return hit.data;
 const queries=["trending","best seller","viral","new launch","deals"];
 const sources=["amazon","flipkart","meesho"];
 let all=[];
 for(const s of sources){
   for(const q of queries){
     try{all.push(...await marketplaceSearch(s,q));}catch(e){console.warn("product source",s,q,e.message)}
     if(all.filter(x=>x.marketplace.toLowerCase().includes(s==="amazon"?"amazon":s)).length>=12)break;
   }
 }
 const seen=new Set(),now=new Date().toUTCString(),rows=[];
 for(const x of all){
   const k=x.title.toLowerCase();if(seen.has(k))continue;seen.add(k);
   ...
   rows.push(productEnrich({id:`p-${hash(x.marketplace+x.title)}`,title:x.title,marketplace:x.marketplace,traffic:synthetic,trafficText:`${x.marketplace} signal`,published:now,ageHours:a,score:Math.round(sc),growth:clamp(280-(x.rank||1)*7,40,300),competition:(x.rank||99)<=8?"Low":"Medium",stage:(x.rank||99)<=5?"Early Rising":"Rising",source:x.marketplace,sourceUrl:x.url,verifiedSignal:true}));
 }
 rows.sort((a,b)=>b.score-a.score);
 const data=rows.slice(0,40);CACHE.set(key,{at:Date.now(),data});return data;
}
...
async function youtubeSearch(query){
 const html=await fetchText(`https://www.youtube.com/results?search_query=${encodeURIComponent(query)}&hl=en&gl=IN`);
...
async function getVideos(){
 const key="videos:IN",hit=CACHE.get(key);if(hit&&Date.now()-hit.at<TTL)return hit.data;
 const queries=["trending india","viral india","shorts india","reels india"];
 let raw=[];
 for(const q of queries){
   try{raw.push(...(await youtubeSearch(q)));}catch(e){console.warn("youtube",e.message)}
 }
 // Direct public attempts only; no news/search-engine substitution.
 for(const platform of ["Instagram","Facebook"]){
   for(const q of ["trending india","viral india"]){
...
app.get("/api/health",(q,r)=>r.json({ok:true,version:"2.2",productSources:["Amazon India","Flipkart","Meesho"],videoSources:["YouTube","Instagram","Facebook"]}));
app.get("/api/feed",async(q,r)=>{try{
 const geo=String(q.query.geo||"IN").toUpperCase(),type=q.query.type||"search";
 let data=type==="product"?(geo==="IN"?await getProducts():[]):type==="video"?(geo==="IN"?await getVideos():[]):await getSearchTrends(geo);
...
app.listen(PORT,()=>console.log(`TrendPulse V2.2 running on ${PORT}`));
